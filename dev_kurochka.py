...
# Здесь !second_exit, чтобы не цеплялся основной бот
    elif ( message.content.startswith("!dev_exit") and (is_this_Snezhinka) ):
        ...
        if (message.content.startswith("!del_join_channel") and ((is_this_Snezhinka)
                                                                 or (message.author.id == message.guild.owner.id))):
            await discobot.functions.del_notification_channel(server_id, channel_id, "join", message.content)

        if (message.content.startswith("!del_leave_channel") and ((is_this_Snezhinka)
                                                                  or (message.author.id == message.guild.owner.id))):
            await discobot.functions.del_notification_channel(server_id, channel_id, "leave", message.content)

        if (message.content.startswith("!del_welcome_channel") and ((is_this_Snezhinka)
                                                                    or (message.author.id == message.guild.owner.id))):
            await discobot.functions.del_notification_channel(server_id, channel_id, "welcome", message.content)
